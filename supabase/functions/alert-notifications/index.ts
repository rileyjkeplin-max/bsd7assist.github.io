...
Deno.serve(async request => {
  ...
  try {
    ...
    if (["config", "status", "subscribe", "unsubscribe", "test-device", "status-expo", "subscribe-expo", "unsubscribe-expo"].includes(action) && profile.access_status === "revoked") {
    ...
    if (action === "test-device") {
      const endpoint = cleanEndpoint(input.endpoint);
      const { data: row, error } = await admin.from("push_subscriptions")
        .select("id,endpoint,p256dh,auth,failure_count")
        .eq("user_id", user.id).eq("endpoint", endpoint).eq("enabled", true).maybeSingle();
      if (error) throw error;
      if (!row) return json({ ok: true, delivered: 0, failed: 0 });
      const keys = await vapidKeys(admin);
      ...
      try {
        await webpush.sendNotification(
          ...
          JSON.stringify({ title: "BSD7 Assist test", body: "This phone is set up to receive BSD7 Assist notifications." }),
          { TTL: 300, urgency: "high" },
        );
        await admin.from("push_subscriptions").update({
          failure_count: 0, last_success_at: new Date().toISOString(), updated_at: new Date().toISOString(),
        }).eq("id", row.id);
        return json({ ok: true, delivered: 1, failed: 0 });
      } catch (cause) {
        const statusCode = Number((cause as { statusCode?: number })?.statusCode || 0);
        const failureCount = Number(row.failure_count || 0) + 1;
        await admin.from("push_subscriptions").update({
          enabled: ![404, 410].includes(statusCode) && failureCount < 3,
          failure_count: failureCount,
          updated_at: new Date().toISOString(),
        }).eq("id", row.id);
        return json({ ok: true, delivered: 0, failed: 1 });
      }
    }
